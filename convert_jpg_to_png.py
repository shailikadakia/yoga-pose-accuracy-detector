...
output_folder = "training-data/warrior_two"

os.makedirs(output_folder, exist_ok=True)

# 1. Find the highest existing index in the output folder
existing_files = [f for f in os.listdir(output_folder) if f.startswith("warrior_two") and f.endswith(".png")]
existing_indices = []

for f in existing_files:
    try:
        num = int(f.replace("warrior_two_", "").replace(".png", ""))
...
for filename in os.listdir(input_folder):
    if filename.lower().endswith(".jpg") or filename.lower().endswith(".jpeg"):
        ...
        new_name = f"warrior_two_{counter}.png"
